/**
 * Phase 49 — PP `pts_rebs_asts` / `pts+rebs+asts` → `pra` (STAT_MAP parity with `fetch_props.mapStatType`).
 */
...
function makeOddsRow(overrides: Partial<InternalPlayerPropOdds> = {}): InternalPlayerPropOdds {
  return {
    ...
    opponent: "LAL",
    league: "NBA",
    stat: "pra" as InternalPlayerPropOdds["stat"],
    line: 42.5,
    overOdds: -110,
    underOdds: -110,
    book: "fanduel",
...
describe("Phase 49 PP PRA combo stat aliases → pra", () => {
  it("matches OddsAPI pra when pick stat is pts_rebs_asts (PrizePicks attr shape)", async () => {
    const odds = [makeOddsRow({ stat: "pra" as InternalPlayerPropOdds["stat"], line: 42.5 })];
    const picks = [makePick({ stat: "pts_rebs_asts" as RawPick["stat"], line: 42.5 })];
  ...
  it("matches OddsAPI pra when pick stat is pts+rebs+asts", async () => {
    const odds = [makeOddsRow({ stat: "pra" as InternalPlayerPropOdds["stat"], line: 42.5 })];
    const picks = [makePick({ stat: "pts+rebs+asts" as RawPick["stat"], line: 42.5 })];
    const { odds: merged } = await mergeWithSnapshot(picks, odds, META, undefined, getDefaultCliArgs());
    expect(merged).toHaveLength(1);
  });
});

describe("Phase 49 archive diff fixtures (A vs B)", () => {
  const left = path.join(__dirname, "fixtures/merge_archive_phase49/snapshot_A_baseline");
  const right = path.join(__dirname, "fixtures/merge_archive_phase49/snapshot_B_after_stat_alias");
  ...
  it("shows higher mergeCoverage and fewer no_odds_stat drops (representative post-fix)", () => {
    ...
    expect(d.diagnosticsByStatCanonicalDeltaLines.some((l) => l.includes("pts_rebs_asts"))).toBe(true);
  });
});
