/**
 * Phase 19D — CLI bankroll, strict parse, effective config, optional dist spawn (migrated from
 * src/__tests__/cli_bankroll.test.ts and src/__tests__/cli_strict_effective_config.test.ts).
 */
import { spawnSync } from "child_process";
import path from "path";
import fs from "fs";
import { parseArgs, getEffectiveConfig } from "../src/cli_args";

const distScript = path.join(process.cwd(), "dist", "src", "run_optimizer.js");
const hasDist = (): boolean => fs.existsSync(distScript);

describe("Phase 19D — CLI bankroll", () => {
  it("parses --bankroll 600", () => {
    expect(parseArgs(["--bankroll", "600"]).bankroll).toBe(600);
  });
  ...
  it("defaults to 600 when no --bankroll (parseCliArgsImpl initial bankroll)", () => {
    expect(parseArgs([]).bankroll).toBe(600);
  });
});

describe("Phase 19D — CLI strict parsing (parseArgs only)", () => {
...
describe("Phase 19D — getEffectiveConfig", () => {
  it("getEffectiveConfig returns expected fields", () => {
    ...
    expect(config.minLegEv).toBe(0.02);
    ...
    expect(config.bankroll).toBe(600);
  });
...
describe("Phase 19D — CLI strict spawn (dist/run_optimizer.js)", () => {
