/**
 * Phase 19D — Exact-line merge (migrated from src/__tests__/exact_line_merge.test.ts).
 */
...
describe("exact-line merge (Prompt 3)", () => {
  ...
  it("exact match takes priority over nearer alt line", async () => {
    ...
    const { odds: merged } = await mergeWithSnapshot([pick], odds, META, undefined, getDefaultCliArgs());
  ...
  it("nearest fallback when no exact match, within tolerance", async () => {
    const odds = [makeOddsRow({ line: 24.5, overOdds: -110, underOdds: -110 })];
