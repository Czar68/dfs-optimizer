/**
 * Phase 19D — Odds snapshot manager and identity (migrated from src/__tests__/odds_snapshot.test.ts).
 */
import {
  OddsSnapshotManager,
  _resolveRefreshMode,
  _writeState,
} from "../src/odds/odds_snapshot_manager";
import {
  generateSnapshotId,
  hashRequestParams,
  computeAgeMinutes,
...
function makeFakeRows(n: number): InternalPlayerPropOdds[] {
...
const mockFetchFn = async (_sports: Sport[], _opts: { forceRefresh: boolean }): Promise<InternalPlayerPropOdds[]> => {
  networkCallCount++;
