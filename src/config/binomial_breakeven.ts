...
// Core math delegated to math_models/breakeven_binomial (locked-down canonical source)
// Table building, formatting, and lookup helpers remain here (application logic).

import { ALL_STRUCTURES } from "./parlay_structures";
import {
  expectedReturnBinomial,
  solveBreakevenProbability,
  probToAmerican,
} from '../../math_models/breakeven_binomial';

export { expectedReturnBinomial, solveBreakevenProbability, probToAmerican };
...
export function breakevenTableMarkdown(): string {
  ...
  lines.push("- UD 2-pick Standard (3.5x): BE ~ 53.45%, American ~ -115.");
  lines.push("- PP 6-pick Flex (25x/2x/0.4x): BE ~ 54.21%, American ~ -118.6.");
  return lines.join("\n");
}
