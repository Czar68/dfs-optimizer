...
`);
  ...
  if (top5.length === 0) {
    console.log("[Telegram] No cards to push.");
    return;
  }
  ...
  const totalKelly = cards.reduce((s, c) => s + c.kellyFrac, 0);

  // Header
  const header = [
    `🏀 <b>Innovative Cards — ${esc(date)}</b>`,
    `━━━━━━━━━━━━━━━━━━━━━`,
    `Portfolio: <b>${cards.length} cards</b> | Kelly total: <b>${(totalKelly * 100).toFixed(1)}%</b> / $${Math.round(totalKelly * bankroll)}`,
  ...
  const footer = [
    ...
    `━━━━━━━━━━━━━━━━━━━━━`,
    `<pre>${esc(statBar)}</pre>`,
    `EV Top: <b>+${topEV}%</b> | Cards: ${cards.length}${sheetLine}`,
    `━━━━━━━━━━━━━━━━━━━━━`,
