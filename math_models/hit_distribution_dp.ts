 ...
 */
export function computeHitDistributionDP(probs: number[]): number[] {
 ...
 */
export function computeHitDistributionRecord(probs: number[]): CardHitDistribution {
  const arr = computeHitDistributionDP(probs);
  const out: CardHitDistribution = {};
  arr.forEach((prob, k) => {
    if (prob > 0) out[k] = prob;
  });
  return out;
}
 ...
 */
export function computePrizePicksHitDistributionIid(
  legTrueProbs: number[],
  _flexType: string
): CardHitDistribution {
