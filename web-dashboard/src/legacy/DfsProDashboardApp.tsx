...
const TIER_STYLE: Record<string, string> = {
  ...
  skip:      'bg-[var(--dfs-surface-3)]/90 text-[color:var(--dfs-text-faint)] ring-1 ring-[color:var(--dfs-border-dim)]',
...
function DfsProDashboardApp() {
  ...
  const DATA_BASE = resolveOptimizerDataBase()
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      if (ppRes.status === 'fulfilled') ppCards = ppRes.value.map(normalizeCardRow).filter((c): c is Card => c != null)
  ...
  if (canonicalSamplesView) {
    return (
      <div className="min-h-screen" style={{ background: 'var(--dfs-bg)', color: 'var(--dfs-text)', fontFamily: 'var(--font-body)' }}>
        <AppHeader
        ...
        <main className="max-w-3xl mx-auto px-4 py-8 space-y-4">
          <p className="text-sm" style={{ color: 'var(--dfs-text-muted)' }}>
            Query: <code style={{ color: 'var(--dfs-text)' }}>?view=canonical-samples</code> — loads JSON from{" "}
            <code style={{ color: 'var(--dfs-text)' }}>public/data/canonical_samples/</code> via the Phase 22 consumer path.
          </p>
  ...
  // ── Tier badge helper (new visual style) ──────────────────────
  const tierBadge = (tier?: string) => {
    const map: Record<string, { cls: string; label: string }> = {
      must_play: { cls: 'tier-must',    label: 'Must' },
      strong:    { cls: 'tier-strong',  label: 'Strong' },
      small:     { cls: 'tier-small',   label: 'Small' },
      lottery:   { cls: 'tier-lottery', label: 'Lottery' },
      skip:      { cls: 'tier-skip',    label: 'Skip' },
      core:      { cls: 'tier-strong',  label: 'Strong' },
    }
    const t = map[tier ?? ''] ?? map.skip
    return (
      <span style={{ display:'inline-block', padding:'2px 8px', borderRadius:'999px', fontSize:'0.65rem', fontWeight:500 }} className={t.cls}>
        {t.label}
      </span>
    )
  }
  ...
  const siteBadge = (site?: string) => {
    const cls = (site ?? '').toUpperCase() === 'PP' ? 'site-pp' : 'site-ud'
    return (
      <span style={{ display:'inline-block', padding:'1px 6px', borderRadius:'3px', fontSize:'0.65rem', fontWeight:500, textAlign:'center' }} className={cls}>
        {site}
      </span>
    )
  }
  ...
  return (
    <div className="min-h-screen antialiased" style={{ background: 'var(--dfs-bg)', color: 'var(--dfs-text)', fontFamily: 'var(--font-body)' }}>

      {/* ── Header ────────────────────────────────────────────── */}
      <header className="dfs-header">
        <div style={{ maxWidth: '1800px', margin: '0 auto', padding: '0 1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '48px', gap: '1rem' }}>
          {/* Brand */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: 28, height: 28, borderRadius: 8, background: 'radial-gradient(circle at 20% 0, #22c55e 0, #166534 40%, #020617 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 0 1px rgba(34,197,94,0.25)' }}>
              <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M8 2L13 6V14H3V6L8 2Z" stroke="#ecfdf5" strokeWidth="1.5" strokeLinejoin="round"/><path d="M6 14V9H10V14" stroke="#ecfdf5" strokeWidth="1.5" strokeLinejoin="round"/></svg>
            </div>
            <div>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '0.85rem', fontWeight: 600, lineHeight: 1.1 }}>DFS PRO</div>
              <div style={{ fontSize: '0.58rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--dfs-text-faint)' }}>Slip Optimizer</div>
            </div>
          </div>

          {/* Page nav */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <DashboardPageNav active={dashboardPage} onChange={navigateDashboardPage} />
          </div>

          {/* Status strip */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px', fontSize: '0.7rem', fontFamily: 'var(--font-mono)', color: 'var(--dfs-text-muted)' }}>
            <span>PP <span style={{ color: 'var(--dfs-text)', fontWeight: 500 }}>{loadStats.pp}</span></span>
            <span>UD <span style={{ color: 'var(--dfs-text)', fontWeight: 500 }}>{loadStats.ud}</span></span>
            {freshAgo && (
      ...
      <main style={{ maxWidth: '1800px', margin: '0 auto', padding: '12px 1rem 2rem' }}>

        {/* ── Overview / Diagnostics panels ─────────────────── */}
        ...
        {/* ── Diagnostics: match coverage ────────────────────── */}
        {dashboardPage === 'diagnostics' && (
          <section
            className="dfs-card"
            style={{ marginTop: 12, padding: '12px', fontSize: '0.72rem' }}
            data-testid="match-coverage-diagnostics"
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <div style={{ fontSize: '0.78rem', fontWeight: 500 }}>Match coverage quality</div>
              <button
                type="button"
                onClick={() => setIsAdminMetricsView(v => !v)}
                className="dfs-btn dfs-btn-ghost"
            ...
            <div style={{ color: 'var(--dfs-text-muted)', marginBottom: 8, fontSize: '0.7rem' }}>{matchMetrics.takeaway}</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6, marginBottom: 8 }}>
              {[
                { label: 'Coverage', val: `${(matchMetrics.rate * 100).toFixed(1)}%` },
                { label: 'Matched', val: matchMetrics.matched },
                { label: 'Unmatched', val: matchMetrics.unmatched },
              ].map(({ label, val }) => (
                <div key={label} className="metric-tile" style={{ textAlign: 'center' }}>
                  <div className="metric-tile-label">{label}</div>
                  <div className="metric-tile-value">{val}</div>
                </div>
              ))}
            </div>
            {!isAdminMetricsView ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {matchMetrics.reasonRows.slice(0, 4).map(([code, count]) => {
                  ...
                  return (
                    <div key={code} className="dfs-inner" style={{ padding: '6px 8px' }}>
                      <div style={{ fontSize: '0.7rem' }}>{meta.label}: <span style={{ color: 'var(--dfs-text)' }}>{count}</span></div>
                      <div style={{ fontSize: '0.65rem', color: 'var(--dfs-text-faint)', marginTop: 2 }}>{meta.help}</div>
                    </div>
                  )
                })}
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div>
                  <div style={{ fontSize: '0.65rem', color: 'var(--dfs-text-faint)', marginBottom: 4 }}>Reason diagnostics</div>
                  <div style={{ maxHeight: 96, overflowY: 'auto', borderRadius: 6, border: '1px solid var(--dfs-border-dim)' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.65rem' }}>
                      <thead style={{ background: 'var(--dfs-surface-3)', color: 'var(--dfs-text-faint)' }}>
                        <tr><th style={{ padding: '4px 6px', textAlign: 'left' }}>Code</th><th style={{ padding: '4px 6px', textAlign: 'left' }}>Label</th><th style={{ padding: '4px 6px', textAlign: 'right' }}>Count</th></tr>
                      </thead>
                      <tbody>
                        {matchMetrics.reasonRows.map(([code, count]) => (
                          <tr key={code} style={{ borderTop: '1px solid var(--dfs-border-dim)' }}>
                            <td style={{ padding: '3px 6px', color: 'var(--dfs-text-muted)' }}>{code}</td>
                            <td style={{ padding: '3px 6px', color: 'var(--dfs-text-muted)' }}>{REASON_LABELS[code]?.label ?? code}</td>
                            <td style={{ padding: '3px 6px', textAlign: 'right' }}>{count}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div>
                  <div style={{ fontSize: '0.65rem', color: 'var(--dfs-text-faint)', marginBottom: 4 }}>Unmatched odds by book</div>
                  <div className="dfs-inner" style={{ maxHeight: 80, overflowY: 'auto', padding: '6px 8px' }}>
                    {matchMetrics.books.length === 0 && <div style={{ fontSize: '0.65rem', color: 'var(--dfs-text-faint)' }}>No book-level diagnostics available.</div>}
                    {matchMetrics.books.map(([book, count]) => (
                      <div key={book} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.68rem', color: 'var(--dfs-text-muted)' }}><span>{book}</span><span>{count}</span></div>
                    ))}
                  </div>
                </div>
              </div>
        ...
        {/* ── Explore: main 3-col grid ────────────────────────── */}
        {dashboardPage === 'explore' && (
          <div style={{ display: 'grid', gridTemplateColumns: '220px minmax(0,1fr) 300px', gap: 12, alignItems: 'start' }}>

            {/* Left sidebar */}
            <aside style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>

              {/* Filters */}
              <div className="dfs-card" style={{ padding: 12 }}>
                <div className="dfs-label" style={{ marginBottom: 8 }}>Filters</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <select className="dfs-select" onChange={e => setSportFilter(e.target.value)} value={sportFilter}>
                    <option>All</option><option>NBA</option><option>NCAAB</option><option>NHL</option><option>NFL</option><option>MLB</option>
                  </select>
                  <select className="dfs-select" onChange={e => setSiteFilter(e.target.value as 'All' | 'PP' | 'UD')} value={siteFilter}>
                    <option value="All">All sites</option><option value="PP">PrizePicks</option><option value="UD">Underdog</option>
                  </select>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.72rem', color: 'var(--dfs-text-muted)', cursor: 'pointer' }}>
                    <input type="checkbox" checked={hideStartedGames} onChange={e => setHideStartedGames(e.target.checked)} />
                    Hide started games
                  </label>
                </div>

                {/* Game pills */}
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--dfs-border-dim)' }}>
                  <div className="dfs-label" style={{ marginBottom: 6 }}>Game window</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                    {gamePills.pills.length === 0 && (
                      <span style={{ fontSize: '0.65rem', color: 'var(--dfs-text-faint)' }}>No game groupings found.</span>
                    )}
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            borderRadius: 999,
                            border: '1px solid',
                            cursor: pill.isStarted ? 'not-allowed' : 'pointer',
                            background: pill.isStarted ? 'transparent' : selected ? 'rgba(34,197,94,0.15)' : 'var(--dfs-surface-3)',
                            borderColor: pill.isStarted ? 'var(--dfs-border-dim)' : selected ? 'rgba(34,197,94,0.35)' : 'var(--dfs-border-dim)',
                            color: pill.isStarted ? 'var(--dfs-text-faint)' : selected ? '#4ade80' : 'var(--dfs-text-muted)',
                            fontFamily: 'var(--font-body)',
                          }}
                          title={pill.isStarted ? 'Started game (unusable)' : `${pill.cardCount} cards`}
                        >
                          {pill.label} {pill.isStarted ? '' : `(${pill.cardCount})`}
                        </button>
                      )
                    })}
                  </div>
                  <div style={{ marginTop: 6, display: 'flex', justifyContent: 'space-between', fontSize: '0.62rem', color: 'var(--dfs-text-faint)' }}>
                    <span>{selectedGameKeys.length > 0 ? `${selectedGameKeys.length} selected` : 'All games'}</span>
                    {selectedGameKeys.length > 0 && (
                      <button type="button" style={{ background: 'none', border: 'none', color: 'var(--dfs-text-muted)', cursor: 'pointer', fontSize: '0.62rem', fontFamily: 'var(--font-body)', textDecoration: 'underline' }} onClick={() => setSelectedGameKeys([])}>
                        Clear
                      </button>
                    )}
                  </div>
                  {gamePills.malformedCount > 0 && (
                    <div style={{ fontSize: '0.62rem', color: 'var(--dfs-text-faint)', marginTop: 4 }}>
                      {gamePills.malformedCount} malformed suppressed
                    </div>
                  )}
                </div>
                ...
                {/* Export */}
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--dfs-border-dim)' }}>
                  <button type="button" onClick={() => exportTableCsv()} className="dfs-btn dfs-btn-ghost" style={{ width: '100%', justifyContent: 'flex-start', fontSize: '0.7rem' }}>
                    Export CSV ↓
                  </button>
                  {copyStatus && <div style={{ marginTop: 4, fontSize: '0.68rem', color: 'var(--dfs-green-text)' }}>{copyStatus}</div>}
                </div>
              </div>

              {/* CSV snapshot */}
              <div className="dfs-card" style={{ padding: 12 }}>
                <div className="dfs-label" style={{ marginBottom: 8 }}>Data snapshot</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: '0.72rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--dfs-text-muted)' }}>PP cards</span>
                    <span className="dfs-mono">{loadStats.pp}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--dfs-text-muted)' }}>PP legs</span>
                    <span className="dfs-mono">{loadStats.ppLegs}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--dfs-text-muted)' }}>UD cards</span>
                    <span className="dfs-mono">{loadStats.ud}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--dfs-text-muted)' }}>UD legs</span>
                    <span className="dfs-mono">{loadStats.udLegs}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 5, borderTop: '1px solid var(--dfs-border-dim)', marginTop: 2 }}>
                    <span style={{ color: 'var(--dfs-text-muted)' }}>Bankroll</span>
                    <span className="dfs-mono">${manifest?.bankroll ?? BANKROLL_DEFAULT}</span>
                  </div>
                  {lastRefreshMs > 0 && (
                    <div style={{ fontSize: '0.62rem', color: 'var(--dfs-text-faint)' }}>
              ...
              {/* Portfolio */}
              <div className="dfs-card" style={{ padding: 12 }}>
                <div className="dfs-label" style={{ marginBottom: 8 }}>Portfolio</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: '0.72rem' }}>
                  {[
                    { label: 'Top 10', val: `$${portfolio.top10stake.toFixed(0)}` },
                    { label: 'Top 20', val: `$${portfolio.top20stake.toFixed(0)}` },
                    { label: 'Total', val: `$${portfolio.totalStake.toFixed(0)} (${portfolio.count})` },
                  ].map(({ label, val }) => (
                    <div key={label} style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: 'var(--dfs-text-muted)' }}>{label}</span>
                      <span className="dfs-mono">{val}</span>
                    </div>
                  ))}
                  <div style={{ paddingTop: 5, borderTop: '1px solid var(--dfs-border-dim)', marginTop: 2, fontSize: '0.62rem', color: 'var(--dfs-text-faint)' }}>
                    Must {tierCounts.must_play} · Strong {tierCounts.strong} · Lot {tierCounts.lottery}
                  </div>
                </div>
              </div>
            </aside>
            ...
            <section style={{ display: 'flex', flexDirection: 'column', gap: 8, minWidth: 0 }}>
              {/* Tab row + copy link */}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--dfs-border-dim)' }}>
                ...
                <button
                  ...
                  className="dfs-btn dfs-btn-ghost"
                  style={{ fontSize: '0.65rem', marginLeft: 8, marginBottom: 4 }}
              ...
              {/* Content views */}
              <div className="dfs-card" style={{ padding: 0, overflow: 'hidden' }}>
                {activeTab === 'tracker' ? (
              ...
              {/* Footer hint */}
              <div style={{ fontSize: '0.62rem', color: 'var(--dfs-text-faint)', display: 'flex', flexWrap: 'wrap', gap: '0 16px' }}>
                {activeTab === 'tracker' ? (
                  <span>Grade legs (Win/Loss/Push) · saves to data/tracking/pending_cards.json</span>
                ) : (activeTab === 'top_legs_pp' || activeTab === 'top_legs_ud') ? (
            ...
            {/* Right sidebar: card detail + results */}
            <aside style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>

              {/* Card detail */}
              <div className="dfs-card" style={{ padding: 12 }}>
                <div className="dfs-label" style={{ marginBottom: 10 }}>Card detail</div>
                {!selectedCard ? (
                  <div style={{ fontSize: '0.72rem', color: 'var(--dfs-text-faint)' }}>Select a row to inspect.</div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {/* Player / prop */}
                    <div>
                      <div style={{ fontSize: '0.8rem', fontWeight: 500, marginBottom: 2 }}>
                        {resolvePlayerPropLine(selectedCard, legs)}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        {siteBadge(selectedCard.site)}
                        {tierBadge(selectedCard.bestBetTier)}
                      </div>
                    </div>

                    {/* Strength bar */}
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.65rem', color: 'var(--dfs-text-faint)', marginBottom: 3 }}>
                        <span>Slip strength</span>
                        <span className="dfs-mono" style={{ color: 'var(--dfs-green-text)' }}>{(Number(selectedCard.bestBetScore) || 0).toFixed(0)}</span>
                      </div>
                      <div className="strength-track">
                        <div className="strength-fill" style={{ width: `${Math.min(100, Number(selectedCard.bestBetScore) || 0)}%` }} />
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
                      {[
                        { label: 'EV', val: `${(Number(selectedCard.cardEv) * 100).toFixed(1)}%`, green: true },
                        { label: 'Win%', val: selectedCard.winProbCash != null ? `${(selectedCard.winProbCash * 100).toFixed(1)}%` : '—', green: false },
                        { label: 'Edge', val: `${(Number(selectedCard.avgEdgePct) <= 1 ? Number(selectedCard.avgEdgePct) * 100 : Number(selectedCard.avgEdgePct)).toFixed(1)}%`, green: false },
                        { label: 'Kelly', val: `$${portfolio.displayedStake(selectedCard, selectedCard.kellyStake).toFixed(2)}`, green: false },
                      ].map(({ label, val, green }) => (
                        <div key={label} className="metric-tile">
                          <div className="metric-tile-label">{label}</div>
                          <div className="metric-tile-value" style={green ? { color: 'var(--dfs-green-text)' } : {}}>{val}</div>
                        </div>
                      ))}
                    </div>

                    {/* Actions */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 5, paddingTop: 6, borderTop: '1px solid var(--dfs-border-dim)' }}>
                      <button type="button" onClick={(e) => copyParlay(selectedCard, e)} className="dfs-btn dfs-btn-primary" style={{ width: '100%' }}>
                        Copy parlay
                      </button>
                      <a
                        href={selectedCard.site === 'UD' ? udFullSlipUrl(getLegIds(selectedCard)) : PP_PROJECTIONS}
                        target="_blank"
              ...
              <div className="dfs-card" style={{ padding: 12 }}>
                <button
                  ...
                  style={{ width: '100%', textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontFamily: 'var(--font-body)', color: 'var(--dfs-text-muted)', padding: 0 }}
                >
                  <span className="dfs-label">Results snapshot</span>
                  <span style={{ fontSize: '0.65rem', color: 'var(--dfs-text-faint)' }}>{expandedResultsPast ? '▼' : '▶'}</span>
                </button>
                {expandedResultsPast && (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 5, marginTop: 10 }}>
                    {[
                      { label: 'Day', box: resultsSummary.day },
                      { label: 'Wk', box: resultsSummary.week },
                      { label: 'Mo', box: resultsSummary.month },
                      { label: 'LT', box: resultsSummary.lt },
                      { label: 'All', box: resultsSummary.past },
                    ].map(({ label, box }) => (
                      <div key={label} className="metric-tile" style={{ textAlign: 'center' }}>
                        <div className="metric-tile-label">{label}</div>
                        <div className="metric-tile-value" style={{ fontSize: '0.72rem' }}>{box.hits}/{box.total}</div>
                      </div>
                    ))}
                  </div>
                )}
...
export default DfsProDashboardApp
