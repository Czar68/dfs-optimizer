...
- runnerLegEligibility: {"adjustedEvThreshold":0.0225,"maxLegsPerPlayerGlobal":1,"minEdgePerLeg":0.015,"minLegEv":0.02,"volumeMode":false}
- legGates: {"adjustedEvThreshold":0.0225,"effectiveEvDefinition":"adjEv ?? legEv","maxLegsPerPlayerGlobal":1,"minEdgePerLeg":0.015,"minLegEv":0.02,"volumeMode":false}
...
- ppEngineWrapper: {"evAdjThresh":0.0225,"maxLegsPerPlayer":1,"minEdge":0.015,"minLegEv":0.02}
...
- note: Engine snapshot: minEdge=0.015 minLegEv=0.02 evAdjThresh=0.0225 maxLegsPerPlayer=1
...
- key=legGates.minEdgePerLeg_vs_udMinEdge relation=intentionally_different pp=0.015 ud=0.006 — PP min edge vs UD default min edge; both honor cliArgs.minEdge when set.
- key=legGates.minLegEv_vs_udMinLegEvForCardBuilder relation=intentionally_different pp=0.02 ud=0.012 — Different defaults and UD uses udMinEv/minEv chain; see Phase 17I audit.

## 6. Notes
- Full comparison (all classifications):
  - [platform_specific_approved] legGates.adjustedEvThreshold_vs_udMinLegEvForCardBuilder (intentionally_different): pp=0.0225 ud=0.012
  - [platform_specific_approved] legGates.maxLegsPerPlayerGlobal_vs_maxLegsPerPlayerPerStat (intentionally_different): pp=1 ud=1
  - [platform_specific_needs_review] legGates.minEdgePerLeg_vs_udMinEdge (intentionally_different): pp=0.015 ud=0.006
  - [platform_specific_needs_review] legGates.minLegEv_vs_udMinLegEvForCardBuilder (intentionally_different): pp=0.02 ud=0.012
