# Fixed comparison script for PP vs UD trueProb values
...
function Get-LegData {
    ...
    # Find column indices (different for PP vs UD)
...
function Compare-TrueProb {
    Write-Host "=== TRUEPROB COMPARISON ANALYSIS (FIXED) ===" -ForegroundColor Green
    Write-Host ""
    ...
    # Show sample data
    Write-Host "PP sample data:" -ForegroundColor Yellow
    $ppLegs | Select-Object -First 3 | ForEach-Object {
        Write-Output "  $($_.Player) $($_.Stat) $($_.Line): trueProb=$($_.TrueProb.ToString('F6'))"
    }
    Write-Host ""
    
    Write-Host "UD sample data:" -ForegroundColor Yellow
    $udLegs | Select-Object -First 3 | ForEach-Object {
        Write-Output "  $($_.Player) $($_.Stat) $($_.Line): trueProb=$($_.TrueProb.ToString('F6'))"
    }
    Write-Host ""
    ...
    # If no matches, show why
    if ($matches.Count -eq 0) {
        Write-Host "❌ NO MATCHING PROPS FOUND" -ForegroundColor Red
        Write-Host "This could indicate:" -ForegroundColor Red
        Write-Host "  - Different player names between platforms" -ForegroundColor Red
        Write-Host "  - Different stat categories" -ForegroundColor Red
        Write-Host "  - Different line values" -ForegroundColor Red
        Write-Host "  - Different naming conventions" -ForegroundColor Red
        Write-Host ""
        
        # Check for common players
        $ppPlayers = $ppLegs | Select-Object -ExpandProperty Player | Sort-Object -Unique
        $udPlayers = $udLegs | Select-Object -ExpandProperty Player | Sort-Object -Unique
        ...
        if ($commonPlayers.Count -gt 0) {
            Write-Host "Common player names:" -ForegroundColor Yellow
            $commonPlayers | ForEach-Object { Write-Output "  - $_" }
            Write-Host ""
            
            # Show props for common players
            foreach ($player in $commonPlayers) {
                Write-Host "Props for ${player}:" -ForegroundColor White
                Write-Host "  PP props:"
                $ppLegs | Where-Object { $_.Player -eq $player } | ForEach-Object {
                    Write-Output "    $($_.Stat) $($_.Line): trueProb=$($_.TrueProb.ToString('F6'))"
                }
                Write-Host "  UD props:"
                $udLegs | Where-Object { $_.Player -eq $player } | ForEach-Object {
                    Write-Output "    $($_.Stat) $($_.Line): trueProb=$($_.TrueProb.ToString('F6'))"
                }
                Write-Host ""
            }
    ...
    # Final assessment
    if ($significantDifferences.Count -eq 0 -and $matches.Count -gt 0) {
        ...
        Write-Host "❓ CONCLUSION: No matching props found to compare" -ForegroundColor Yellow
        Write-Host "   This may be normal for this slate (different prop availability)" -ForegroundColor Yellow
        Write-Host "   Need to test with a slate that has overlapping props" -ForegroundColor Yellow
    } else {
